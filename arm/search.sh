...
API_KEY=$(az search query-key list --resource-group $NAME --service-name ${NAME}-search | jq -r ".[0].key")
...
curl -H "Content-Type: application/json" -H "api-key: $API_KEY" \
  -s -o $TMPLOC/$$.json \
  -w"\nResponse: %{http_code}\n" \
  "https://${NAME}-search.search.windows.net/indexes/$NAME/docs?api-version=2020-06-30&$QRY"
